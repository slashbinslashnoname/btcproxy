...
func (up *UpSessionBTC) handleSubScribeResponse(rpcData *JSONRPCLineBTC, jsonBytes []byte) {
	...
	up.manager.extranonce = sessionIDHex
	up.manager.miningNotify = mnn[1].(string)
...
func (up *UpSessionBTC) handleSubmitShare(e EventSubmitShareBTC) {
	...
	if e.Message.HasVersionMask != false {
		params = []interface{}{string(e.Message.Login), string(e.Message.JobID), string(e.Message.ExtraNonce2), string(e.Message.Time), string(e.Message.Nonce), string(e.Message.VersionMask)}
	} else {
		params = []interface{}{string(e.Message.Login), string(e.Message.JobID), string(e.Message.ExtraNonce2), string(e.Message.Time), string(e.Message.Nonce)}
	}
